...
function onContext(context) {
  ...
  var headers = {
    "x-chkp-sid": context["management-server-api"]["sid"],
    "Content-Type": "application/json",
    "x-chkp-smc": context["management-server-api"]["url"] + "/" + command
  };
  var params = {};
...
function chainStart(command, params, headers, context) {
  ...
  $.ajax(settings).done(function (response) {
    ...
    $("#searchResults").append(
      "<table width='100%' id='resultsTable'><tr><thead><th>Hostname</th><th>IP Address</th><th>Domain</th></thead></tr><tbody></tbody></table>"
    );
    for (i = 0; i < found_domains.length; i++) {
      ...
      $.ajax(settings).done(function (response) {
        ...
        var params = {
          type: "host",
          "ip-only": "true",
          filter: $("#ipAddress").val()
        };
        var command = "show-objects";
        var headers = {
          "Content-Type": "application/json",
          "x-chkp-sid": response["sid"],
          "x-chkp-smc": context["management-server-api"]["url"] + "/" + command
        };
        var settings = {
          url: SMS_API_ENDPOINT + "/" + command,
        ...
        $.ajax(settings).done(function (response) {
          ...
          for (o = 0; o < objects_found.length; o++) {
            $("#resultsTable").append(
              "<tr><td>" +
              objects_found[o]["name"] +
              "</td>" +
              "<td>" +
              objects_found[o]["ipv4-address"] +
              "</td>"
            );
            for (s = 0; s < found_domains.length; s++) {
              if (
                found_domains[s]["uid"] == objects_found[o]["domain"]["uid"]
              ) {
                $("#resultsTable td:last").after(
                  "<td>" + found_domains[s]["name"] + "</td></tr>"
                );
                break;
              }
            }
